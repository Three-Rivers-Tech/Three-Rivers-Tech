...
TypeScript is the default; prefer strict typing with shared interfaces in `types/`. Follow the ESLint Flat config extending `next/core-web-vitals` and `next/typescript`, which enforces 2-space indentation, JSX import rules, and async best practices. Keep filenames kebab-case, except for React components that stay PascalCase. Import local modules via the `@/` alias so paths remain stable during refactors.
